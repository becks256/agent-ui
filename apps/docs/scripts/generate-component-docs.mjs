...
function parseMarkdownDoc(filePath) {
  ...
  if (propsContent) {
    ...
    for (const line of lines) {
      ...
      if (columns[0].toLowerCase() === "prop" || columns[0].includes("---"))
        continue;
  ...
  if (typesContent) {
    ...
    for (const block of typeBlocks) {
      ...
      const typeName =
        firstLineBreak > -1 ? trimmed.slice(0, firstLineBreak).trim() : "Type";
  ...
  if (basicUsageContent) {
    const codeMatch = basicUsageContent.match(
      /```(?:tsx|jsx|typescript|ts|html|bash)?\r?\n([\s\S]*?)\r?\n```/,
    );
    if (codeMatch) {
  ...
  if (advUsageContent) {
    const codeMatch = advUsageContent.match(
      /```(?:tsx|jsx|typescript|ts|html|bash)?\r?\n([\s\S]*?)\r?\n```/,
    );
    if (codeMatch) {
  ...
  return {
    ...
    importStatement:
      ...
      `import { ${meta.name} } from '@noetic-ui/react';`,
...
export function generateComponentDocs() {
  ...
  console.log(
    `🔍 Discovered ${mdFiles.length} component README markdown files...`,
  );
...
`;
  ...
  console.log(
    `✔ Successfully generated ${outputFile} from ${Object.keys(componentDocs).length} component READMEs!`,
  );
}
