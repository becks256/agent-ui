...
cli
  .command(
    "list",
    "List all available Noetic UI components across all 7 architectural suites",
  )
  .action(() => {
    listComponents();
  });
...
cli
  .command(
    "init",
    "Initialize Noetic UI configuration, utility helpers, types, and CSS variables",
  )
...
cli
  .command(
    "add [...components]",
    "Add one or more Noetic UI components to your project",
  )
  .option("-a, --all", "Add all available components")
  .option(
    "-s, --suite <suite>",
    "Add all components in a specific suite (e.g. reasoning, chat, hitl)",
  )
...
cli
  .command(
    "theme [preset]",
    "Configure the primary color theme and WCAG contrast in globals.css",
  )
  .option("--cwd <cwd>", "Working directory")
