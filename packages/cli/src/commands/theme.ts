...
export async function setTheme(options: { cwd?: string; preset?: string } = {}) {
  ...
  if (options.preset) {
    const matched = THEME_PRESETS.find((p) => p.name.toLowerCase() === options.preset?.toLowerCase());
    if (matched) {
    ...
    if (response.preset === 'custom') {
      const customResponse = await prompts([
        { type: 'number', name: 'hue', message: 'Enter Hue (0 - 360):', initial: 265, min: 0, max: 360 },
        { type: 'number', name: 'sat', message: 'Enter Saturation (0 - 100%):', initial: 85, min: 0, max: 100 },
        { type: 'number', name: 'light', message: 'Enter Lightness (15 - 85%):', initial: 60, min: 15, max: 85 },
      ]);
...
`;
  ...
  if (fs.existsSync(fullCssPath)) {
    ...
    if (existing.includes('--primary-hue')) {
      ...
      existing = existing.replace(/--primary-hue:\s*\d+;/g, `--primary-hue: ${selectedHue};`);
      existing = existing.replace(/--primary-sat:\s*\d+%;/g, `--primary-sat: ${selectedSat}%;`);
      existing = existing.replace(/--primary-light:\s*\d+%;/g, `--primary-light: ${selectedLight}%;`);
      existing = existing.replace(/--primary-foreground:\s*[^;]+;/g, `--primary-foreground: ${fgHsl};`);
