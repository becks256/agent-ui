...
export async function addComponents(
  ...
  const targetPath =
    config.components || config.aliases?.components || "components/agent-ui";
  ...
  if (options.all) {
    ...
    selectedComponents = Object.keys(COMPONENT_REGISTRY).filter((name) => {
      ...
      return (
        meta.suiteId.toLowerCase() === suiteFilter ||
        meta.suite.toLowerCase().includes(suiteFilter)
      );
    });
    ...
    if (selectedComponents.length === 0) {
      ...
      console.log(
        `  Valid suites: reasoning, chat, input, hitl, canvas, telemetry, theme\n`,
      );
      return;
    }
  ...
  if (invalidComponents.length > 0) {
    ...
    console.log(
      `  Run ${chalk.bold("npx agent-ui list")} to see all available components.\n`,
    );
  }
  ...
  for (const compKey of componentsToWrite) {
    ...
    if (fileExists && !options.overwrite) {
      console.log(
        `  ${chalk.dim("ℹ")} Skipped ${chalk.bold(template.filename)} (already exists, use --overwrite to replace)`,
      );
      continue;
    }
    ...
    console.log(
      `  ${chalk.green("✔")} Added ${chalk.bold(path.relative(cwd, destPath))}`,
    );
  }
  ...
  const installCmd = getInstallCommand(
    pkgManager,
    Array.from(externalDependencies),
  );
