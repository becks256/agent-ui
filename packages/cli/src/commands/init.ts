...
export async function initProject(options: InitOptions = {}) {
  ...
  console.log(
    `  ${chalk.green("✔")} Created components directory at ${chalk.bold(config.components)}`,
  );
  ...
  if (!fs.existsSync(cnFilePath)) {
    ...
    console.log(
      `  ${chalk.green("✔")} Created utility helper at ${chalk.bold(path.relative(cwd, cnFilePath))}`,
    );
  }
  ...
  if (!fs.existsSync(typesFilePath)) {
    await fs.writeFile(
      typesFilePath,
      COMPONENT_TEMPLATES["types"].content,
      "utf8",
    );
    console.log(
      `  ${chalk.green("✔")} Created shared agent types at ${chalk.bold(path.relative(cwd, typesFilePath))}`,
    );
  }
  ...
  if (fs.existsSync(fullCssPath)) {
    ...
    if (
      !existingCss.includes("--primary-hue") &&
      !existingCss.includes("--agent-thought")
    ) {
      await fs.appendFile(fullCssPath, CSS_COLOR_VARIABLES, "utf8");
      console.log(
        `  ${chalk.green("✔")} Added Noetic UI color tokens to ${chalk.bold(config.css)}`,
      );
    } else {
      console.log(
        `  ${chalk.dim("ℹ")} CSS color tokens already present in ${chalk.bold(config.css)}`,
      );
    }
  } else {
    ...
    console.log(
      `  ${chalk.green("✔")} Created ${chalk.bold(config.css)} with Noetic UI color tokens`,
    );
  }
  ...
  console.log(
    `     ${chalk.bold.hex("#8b5cf6")("npx @noetic-ui/cli add <component>")}`,
  );
  console.log(
    `     ${chalk.bold.hex("#8b5cf6")("npx @noetic-ui/cli add --all")}\n`,
  );
}
